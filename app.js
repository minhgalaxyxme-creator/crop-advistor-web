...
const sampleButtons = {
  lowland: document.querySelector("#demoLowland"),
  highland: document.querySelector("#demoHighland"),
};
...
sampleButtons.lowland.addEventListener("click", () => {
  selectSample("lowland");
});

sampleButtons.highland.addEventListener("click", () => {
  selectSample("highland");
});
...
function selectSample(sampleKey) {
  ...
  applySample(demoSamples[sampleKey]);
...
function setSelectedSample(sampleKey) {
  selectedSample = sampleKey;
  Object.entries(sampleButtons).forEach(([key, button]) => {
    const isSelected = key === sampleKey;
